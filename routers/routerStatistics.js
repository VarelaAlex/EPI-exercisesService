...
routerStatistics.get("/classroom/:classroomId", async (req, res) => {
    ...
    try {
        ...
        feedbacks.sort((a, b) => {
            const networkTypeComparison = networkTypeOrder.indexOf(a.networkType) - networkTypeOrder.indexOf(b.networkType);
        ...
        feedbacks.forEach((feedback) => {
            ...
            stackedData[representation][networkType] = (stackedData[representation][networkType] || 0) + 1;

            representationCounts[representation] = (representationCounts[representation] || 0) + 1;
        });
        ...
        for (let representation in stackedData) {
            ...
            for (let networkType in stackedData[representation]) {
                ...
                let percentage = ((count / totalInRepresentation) * 100).toFixed(2);
        ...
        for (let representation in representationCounts) {
            ...
            representationPercentages[representation] = ((count / totalFeedbacks) * 100).toFixed(2);
        }
    ...
    try {
        ...
        for (let errorType in iconicMixedErrors) {
            for (let error in iconicMixedErrors[errorType]) {
                ...
                iconicMixedErrors[errorType][error] = {
                    count,
                    percentage: totalIconicMixedErrors > 0 ? ((count / totalIconicMixedErrors) * 100).toFixed(2) : "0.00"
                };
            }
        }
        ...
        for (let errorType in symbolicErrors) {
            ...
            symbolicErrors[errorType] = {
                count,
                percentage: totalSymbolicErrors > 0 ? ((count / totalSymbolicErrors) * 100).toFixed(2) : "0.00"
            };
        }
        ...
        let percentageLexical = (sumLexical / iconicMixedErrorsTotal).toFixed(2);
        let percentageSyntactic = (sumSyntactic / iconicMixedErrorsTotal).toFixed(2);
        let percentageSemantic = (sumSemantic / iconicMixedErrorsTotal).toFixed(2);

        const symbolicErrorsTotal = Object.values(symbolicErrors).reduce((acc, value) => acc + value.count, 0);

        res.status(200).json({ iconicMixedErrors, symbolicErrors, percentageLexical, percentageSyntactic, percentageSemantic, totalFeedbacks, iconicMixedErrorsTotal, symbolicErrorsTotal });
    } catch (e) {
        return res.status(500).json({ error: { type: "internalServerError", message: e.message } });
    }
});
